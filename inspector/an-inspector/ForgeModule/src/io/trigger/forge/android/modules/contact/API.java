...
public class API {
        ...
        ForgeApp.getActivity().requestPermission(Manifest.permission.WRITE_CONTACTS, new EventAccessBlock() {
            ...
            public void run(boolean granted) {
                ...
                try {
                    ...
                    if (result.length > 0) {
                        String rawContactID = String.valueOf(ContentUris.parseId(result[0].uri));
                        String id = Util.ConvertRawContactIdToContactId(rawContactID);
    ...
    private static void addContactWithAccount(final ForgeTask task, final JsonObject contact, String accountName, String accountType) {
        ...
        ArrayList<ContentProviderOperation> person = Util.contactFromJSON(accountType, accountName, contact);

        ContentResolver resolver = ForgeApp.getActivity().getContentResolver();
        ...
        try {
            ...
            task.error("couldn't add contact: " + e,"UNEXPECTED_FAILURE", null);
            return;
        }
        ...
        String rawContactId = String.valueOf(ContentUris.parseId(contactURI));
        String id = Util.ConvertRawContactIdToContactId(rawContactId);
     ...
     */
    @SuppressLint("NewApi")
    public static void insert(final ForgeTask task, @ForgeParam("contact") final JsonObject contact) {
        ForgeApp.getActivity().requestPermission(Manifest.permission.WRITE_CONTACTS, new EventAccessBlock() {
            @Override
            public void run(boolean granted) {
                if (!granted) {
