...
class Util {
    ...
    */
    ...
    /**
     * Returns the last contactId for the given rawContactId
     *
     * @see https://stackoverflow.com/questions/5151885/
     * @see http://android-contact-id-vs-raw-contact-id.blogspot.com
     *
     * @param rawContactId
     * @return
     */

    public static String ConvertRawContactIdToContactId(final String rawContactId) {
        ...
        if (cursor == null || cursor.getCount() <= 0) {
            ForgeLog.d("Could not find contact with raw contact id: " + rawContactId);
            return null;
        }
        ...
        try {
            ...
            do {
                contactId = cursor.getString(cursor.getColumnIndex(ContactsContract.RawContacts.CONTACT_ID));
            } while (cursor.moveToNext());

            return contactId;

        } catch (Exception e) {
            ForgeLog.e("Failed to read contact: " + e.getLocalizedMessage());
            e.printStackTrace();
        } finally {
            cursor.close();
        }
     ...
     */
    public static ArrayList<ContentProviderOperation> contactFromJSON(String accountType, String accountName, JsonObject contact) {
        ...
        // name
        if (Util.HasKeyWithValidValue(contact, "name")) {
            JsonObject name = contact.getAsJsonObject("name");
            ContentProviderOperation.Builder builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
            builder = Util.addName(builder, ContactsContract.CommonDataKinds.StructuredName.PREFIX, name, "honorificPrefix");
            builder = Util.addName(builder, ContactsContract.CommonDataKinds.StructuredName.GIVEN_NAME, name, "givenName");
            builder = Util.addName(builder, ContactsContract.CommonDataKinds.StructuredName.MIDDLE_NAME, name, "middleName");
            builder = Util.addName(builder, ContactsContract.CommonDataKinds.StructuredName.FAMILY_NAME, name, "familyName");
            builder = Util.addName(builder, ContactsContract.CommonDataKinds.StructuredName.SUFFIX, name, "honorificSuffix");
            person.add(builder.build());
        }

        // nickname
        if (Util.HasKeyWithValidValue(contact, "nickname")) {
        ...
        // phone numbers
        if (Util.HasKeyWithValidValue(contact, "phoneNumbers")) {
            ...
            for (JsonElement element : phoneNumbers) {
                ...
                if (Util.HasKeyWithValidValue(phoneNumber, "type") && Util.HasKeyWithValidValue(phoneNumber, "value")) {
        ...
        // emails
        if (Util.HasKeyWithValidValue(contact, "emails")) {
            JsonArray emails = contact.getAsJsonArray("emails");
            for (JsonElement element : emails) {
                JsonObject email = element.getAsJsonObject();
                if (Util.HasKeyWithValidValue(email, "type") && Util.HasKeyWithValidValue(email, "value")) {
        ...
        // addresses
        if (Util.HasKeyWithValidValue(contact, "addresses")) {
            ...
            for (JsonElement element : addresses) {
                ...
                if (Util.HasKeyWithValidValue(address, "type")) {
        ...
        // note
        if (Util.HasKeyWithValidValue(contact, "note")) {
            person.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
                    .withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
    ...
    private static ContentProviderOperation.Builder addName(ContentProviderOperation.Builder builder, String key, JsonObject name, String value) {
        if (!Util.HasKeyWithValidValue(name,  value)) {
            return builder;
        }

        return builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
    ...
    private static void addAddress(ArrayList<ContentProviderOperation> person, JsonObject address) {
        ...
        String streetAddress = Util.HasKeyWithValidValue(address,"streetAddress") ? address.get("streetAddress").getAsString() : "";
        String locality = Util.HasKeyWithValidValue(address,"locality") ? address.get("locality").getAsString() : "";
        String region = Util.HasKeyWithValidValue(address,"region") ? address.get("region").getAsString() : "";
        String postalCode = Util.HasKeyWithValidValue(address,"postalCode") ? address.get("postalCode").getAsString() : "";
        String country = Util.HasKeyWithValidValue(address, "country") ? address.get("country").getAsString() : "";
        person.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
                .withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0)
    ...
    private static boolean HasKeyWithValidValue(JsonObject object, String key) {
